...
async def get_data(sensor_data: SensorsData, db : Session = Depends(get_db)):
    device = db.query(DevicesDataBase).filter_by(device_id = sensor_data.device_MAC).first()
    if not device:
        raise HTTPException(status_code=401, detail="Unauthorized")
    entry = SensorsDataBase(device_id=sensor_data.device_MAC, data = sensor_data.data)
    db.add(entry)
    db.commit()
    print(f"Received data from {sensor_data.device_MAC}: {sensor_data.data}")
    return {"status": "ok"}